...
main() {
    ...
    if $MODE_FLASH; then
        ...
        echo "  flashes Update.stu — the console's numbered buttons do nothing here."
        echo ""
        echo "  Method 1 — auto-flash (primary, no buttons):"
        echo "  1. Leave the system powered ON."
        echo "  2. Insert the USB into the pedestal jack labelled SETUP / SETUP B"
        echo "     / SERVICE (some revisions use Micro-USB — an adapter is needed)."
        echo "  3. Wait 30 s to a few minutes; the pedestal reboots on its own when"
        echo "     the flash completes. Do NOT touch the console buttons."
        echo ""
        echo "  Method 2 — pedestal Control button (fallback):"
        echo "  1. Unplug the AC power cord completely."
        echo "  2. Insert the USB into the SETUP / SETUP B / SERVICE jack."
        echo "  3. Press and HOLD the Control button on the BACK of the pedestal."
        echo "  4. While still holding, reconnect AC power; hold ~5 s, then release."
        echo "  5. Wait up to 5 minutes — pedestal reboots automatically when done."
        echo ""
        echo "  If the pedestal WiFi LED never changes, the stick is not being read:"
        echo "  confirm the SETUP/SERVICE jack (not the network port), FAT32, and try"
        echo "  a different small USB 2.0 drive."
        echo ""
        echo "  After the reflash reboot, remove the USB, then hold the pedestal"
        echo "  Control button (~3 s) → solid amber → join 'Bose Wave ST (…)' WiFi,"
        echo "  open BosMan, enter your home WiFi."
        echo ""
    fi
